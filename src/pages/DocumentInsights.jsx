...
const DocumentInsight = (props) => {  // Accept props
    const [documentData, setDocumentData] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const { leadId: paramLeadId, documentType: paramDocumentType } = useParams(); // Get leadId and documentType from URL

    // Determine if we're in test mode and have props
    const leadId = props.leadId || paramLeadId;
    const documentType = props.documentType || paramDocumentType;
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const result = await axios.get(`http://localhost:8080/documents/${leadId}/${documentType}`); // Use leadId and documentType
                setDocumentData(result.data);
                console.log("Fetched DocumentData:", result.data);
    ...
    const prepareQualityData = () => {
        ...
        try {
            if (documentData?.qualityResults?.qualityAnalysis) {
                console.log("documentData.qualityResults exists:", documentData.qualityResults);
                const qualityAnalysis = documentData.qualityResults.qualityAnalysis; // Directly access qualityAnalysis
    ...
    const prepareForgeryData = () => {
        ...
        try {
            if (documentData?.forgeryResults?.forgeryAnalysis) {
                ...
                console.log("forgeryAnalysis", forgeryAnalysis);
