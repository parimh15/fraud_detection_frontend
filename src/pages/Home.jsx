...
const Home = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const audioResponse = await fetch(`http://localhost:8080/audio/recent/${userId}?limit=5`);
        const documentResponse = await fetch(`http://localhost:8080/documents/recent/${userId}?limit=5`);
